...
-- Dynamically find the Reroll ID in case it changes
for _, dp in ipairs(ItemData.Products) do
	if dp.IsReroll then REROLL_ID = dp.ID; break end
end
...
function ShopTab.Init(parentFrame, tooltipMgr)
	...
	PremiumPanel.Size = UDim2.new(0.48, 0, 0.75, 0); PremiumPanel.Position = UDim2.new(0.01, 0, 0.1, 0); PremiumPanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
	...
	-- THE FIX: Loops over ItemData to build Premium Store dynamically
	for _, gp in ipairs(ItemData.Gamepasses) do
		...
		local btn = Instance.new("TextButton", row); btn.Size = UDim2.new(0, 100, 0, 35); btn.AnchorPoint = Vector2.new(1, 0.5); btn.Position = UDim2.new(1, -10, 0.5, 0); btn.BackgroundColor3 = Color3.fromRGB(120, 80, 150); btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.new(1,1,1); btn.TextSize = 12; btn.Text = "BUY"
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0,4)
		btn.MouseButton1Click:Connect(function() MarketplaceService:PromptGamePassPurchase(player, gp.ID) end)
	end

	for _, dp in ipairs(ItemData.Products) do
		if dp.IsReroll then continue end -- Exclude Reroll from standard list
		local row = Instance.new("Frame", PremList)
		...
		local btn = Instance.new("TextButton", row); btn.Size = UDim2.new(0, 100, 0, 35); btn.AnchorPoint = Vector2.new(1, 0.5); btn.Position = UDim2.new(1, -10, 0.5, 0); btn.BackgroundColor3 = Color3.fromRGB(60, 120, 60); btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.new(1,1,1); btn.TextSize = 12; btn.Text = "BUY"
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0,4)
	...
	SupplyPanel.Size = UDim2.new(0.48, 0, 0.75, 0); SupplyPanel.Position = UDim2.new(0.51, 0, 0.1, 0); SupplyPanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
	...
	TimeLabel.Size = UDim2.new(0.5, 0, 1, 0); TimeLabel.Position = UDim2.new(0.05, 0, 0, 0); TimeLabel.BackgroundTransparency = 1; TimeLabel.Font = Enum.Font.GothamBlack; TimeLabel.TextColor3 = Color3.fromRGB(255, 255, 255); TimeLabel.TextSize = 18; TimeLabel.TextXAlignment = Enum.TextXAlignment.Left
	ApplyGradient(TimeLabel, Color3.fromRGB(255, 100, 100), Color3.fromRGB(255, 200, 100))

	RRBtn = Instance.new("TextButton", Header)
	RRBtn.Size = UDim2.new(0, 140, 0, 30); RRBtn.AnchorPoint = Vector2.new(1, 0.5); RRBtn.Position = UDim2.new(1, -10, 0.5, 0); RRBtn.BackgroundColor3 = Color3.fromRGB(150, 100, 30)
	RRBtn.Font = Enum.Font.GothamBold; RRBtn.TextColor3 = Color3.fromRGB(255,255,255); RRBtn.TextSize = 11; RRBtn.Text = "REROLL (15 R$)"
	...
	local function CheckVIPReroll()
		...
		if hasVIP and os.time() - lastRoll >= 86400 then
			RRBtn.Text = "FREE REROLL (VIP)"; RRBtn.BackgroundColor3 = Color3.fromRGB(200, 160, 40); return true
		else
	...
	local function FetchAndRenderShop()
		...
		for _, item in ipairs(currentShopData.Items) do
			...
			local nLbl = Instance.new("TextLabel", row); nLbl.Size = UDim2.new(0.6, 0, 0, 25); nLbl.Position = UDim2.new(0, 15, 0, 5); nLbl.BackgroundTransparency = 1; nLbl.Font = Enum.Font.GothamBold; nLbl.TextColor3 = Color3.fromRGB(255,255,255); nLbl.TextXAlignment = Enum.TextXAlignment.Left; nLbl.RichText = true; nLbl.TextSize = 13
			...
			local bBtn = Instance.new("TextButton", row); bBtn.Size = UDim2.new(0, 80, 0, 35); bBtn.AnchorPoint = Vector2.new(1, 0.5); bBtn.Position = UDim2.new(1, -10, 0.5, 0); bBtn.BackgroundColor3 = Color3.fromRGB(40, 80, 40); bBtn.Font = Enum.Font.GothamBold; bBtn.TextColor3 = Color3.fromRGB(255,255,255); bBtn.TextSize = 12; bBtn.Text = "BUY"
			Instance.new("UICorner", bBtn).CornerRadius = UDim.new(0,4)

			bBtn.MouseButton1Click:Connect(function()
				Network.ShopAction:FireServer(item.Name)
				bBtn.Text = "BOUGHT"; bBtn.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
				task.delay(1, function() bBtn.Text = "BUY"; bBtn.BackgroundColor3 = Color3.fromRGB(40, 80, 40) end)
			end)
		end
		ShopGrid.CanvasSize = UDim2.new(0, 0, 0, sgLayout.AbsoluteContentSize.Y + 20)
		PremList.CanvasSize = UDim2.new(0, 0, 0, pLayout.AbsoluteContentSize.Y + 20)
	end
	...
	CodePanel.Size = UDim2.new(0.98, 0, 0.12, 0); CodePanel.Position = UDim2.new(0.01, 0, 0.865, 0); CodePanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
	...
	cTitle.Size = UDim2.new(0.2, 0, 1, 0); cTitle.Position = UDim2.new(0.02, 0, 0, 0); cTitle.BackgroundTransparency = 1; cTitle.Font = Enum.Font.GothamBlack; cTitle.TextColor3 = Color3.fromRGB(200, 200, 200); cTitle.TextSize = 16; cTitle.TextXAlignment = Enum.TextXAlignment.Left; cTitle.Text = "ENTER CODE:"

	local cInput = Instance.new("TextBox", CodePanel)
	cInput.Size = UDim2.new(0.5, 0, 0.6, 0); cInput.Position = UDim2.new(0.25, 0, 0.2, 0); cInput.BackgroundColor3 = Color3.fromRGB(15, 15, 18); cInput.Font = Enum.Font.GothamBold; cInput.TextColor3 = Color3.fromRGB(255, 255, 255); cInput.TextSize = 14; cInput.PlaceholderText = "Type code here..."
	...
	cBtn.Size = UDim2.new(0.2, 0, 0.6, 0); cBtn.Position = UDim2.new(0.78, 0, 0.2, 0); cBtn.BackgroundColor3 = Color3.fromRGB(60, 120, 180); cBtn.Font = Enum.Font.GothamBlack; cBtn.TextColor3 = Color3.fromRGB(255, 255, 255); cBtn.TextSize = 14; cBtn.Text = "REDEEM"
