...
local StatusDescs = {
	Stun = "Cannot move or act.", Poison = "Takes damage every turn.", Burn = "Takes damage every turn.", Bleed = "Takes damage every turn.",
	Freeze = "Frozen solid. Cannot move, takes damage.", Confusion = "May attack allies or self.",
	Buff_Strength = "Increased damage dealt.", Buff_Defense = "Reduced damage taken.", Buff_Speed = "Increased evasion and turn priority.", Buff_Willpower = "Increased crit and survival chance.",
	Debuff_Strength = "Reduced damage dealt.", Debuff_Defense = "Increased damage taken.", Debuff_Speed = "Reduced evasion and turn priority.", Debuff_Willpower = "Reduced crit and survival chance."
}
...
local function SyncFighter(fKey, isAlly, id, name, iconId, hp, maxHp, statuses, immunities)
	if not activeFighters[fKey] then
		...
		if f.InfoArea and f.InfoArea:FindFirstChild("NameLabel") then f.InfoArea.NameLabel.Text = name end
	end
	local f = activeFighters[fKey]
	...
	for eff, _ in pairs(StatusIcons) do if not currentStatuses[eff] then f:RemoveStatus(eff) end end

	local hasStunImmunity = (immunities and immunities.Stun and immunities.Stun > 0)
	if hasStunImmunity then f:SetCooldown("StunImmunity", "STN", tostring(immunities.Stun), "Immune to Stun effects.") else f:RemoveCooldown("StunImmunity") end

	local hasConfImmunity = (immunities and immunities.Confusion and immunities.Confusion > 0)
	if hasConfImmunity then f:SetCooldown("ConfImmunity", "CNF", tostring(immunities.Confusion), "Immune to Confusion effects.") else f:RemoveCooldown("ConfImmunity") end
end
...
function StoryTab.Init(parentFrame, tooltipMgr, focusFunc, passedModifierBubble)
		...
		modifierBubble.MouseEnter:Connect(function()
			...
			if modStr == "None" or modStr == "" then
				...
				for _, m in ipairs(string.split(modStr, ",")) do
					...
					if mData then tooltipStr = tooltipStr .. "<b><font color='"..(mData.Color or "#FFFFFF").."'>"..m.."</font></b>\n" .. mData.Description .. "\n\n" end
				end
			end
	...
	resourceLabel.TextColor3 = Color3.fromRGB(255, 100, 50)
	...
	btnLayout.FillDirection = Enum.FillDirection.Horizontal; btnLayout.SortOrder = Enum.SortOrder.LayoutOrder; btnLayout.Padding = UDim.new(0, 15); btnLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center; btnLayout.VerticalAlignment = Enum.VerticalAlignment.Center; btnLayout.Parent = buttonContainer

	local function makeBtn(name, text)
		local btn = Instance.new("TextButton")
		btn.Name = name; btn.Size = UDim2.new(0.28, 0, 0.5, 0); btn.BackgroundColor3 = Color3.fromRGB(30, 20, 50); btn.Text = text; btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.fromRGB(255, 235, 130); btn.TextScaled = true; btn.ZIndex = 23; btn.Parent = buttonContainer
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 8)
		local str = Instance.new("UIStroke", btn); str.Color = Color3.fromRGB(90, 50, 120); str.Thickness = 2; str.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		local ts = Instance.new("UITextSizeConstraint", btn); ts.MaxTextSize = 22; ts.MinTextSize = 10
		return btn
	end

	prestigeBtn = makeBtn("PrestigeBtn", "Prestige")
	prestigeBtn.BackgroundColor3 = Color3.fromRGB(120, 30, 30); prestigeBtn.TextColor3 = Color3.fromRGB(255, 200, 200); prestigeBtn:FindFirstChildOfClass("UIStroke").Color = Color3.fromRGB(200, 50, 50); prestigeBtn.LayoutOrder = 0; prestigeBtn.Visible = false
	...
	local function UpdateStoryUI()
		...
		if modifierBubble then modifierBubble.Visible = (prestige > 0 and parentFrame.Visible) end

		if currentPart >= 8 then randomEncounterBtn.Visible = false; storyEncounterBtn.Visible = false; prestigeBtn.Visible = true
		elseif currentPart == 7 then randomEncounterBtn.Visible = false; storyEncounterBtn.Visible = true; prestigeBtn.Visible = true
		else randomEncounterBtn.Visible = true; storyEncounterBtn.Visible = true; prestigeBtn.Visible = false end
		...
		if partData then
			...
			storyEncounterBtn.Text = (currentMission <= #missionTable and missionTable[currentMission]) and "Story: " .. missionTable[currentMission].Name or "Story Encounter"
		end
	end
	...
	task.spawn(function() local pObj = player:WaitForChild("leaderstats", 5); if pObj then pObj:WaitForChild("Prestige", 5).Changed:Connect(UpdateStoryUI) end end)
	UpdateStoryUI()
end
...
function StoryTab.RenderSkills(battleData)
	...
	local myTitan, myStyle = battleData.Player.Titan or "None", battleData.Player.Style or "None"
	local valid = {}
	for n, s in pairs(SkillData.Skills) do
		if s.Requirement == "None" or s.Requirement == myTitan or s.Requirement == myStyle or (s.Requirement == "AnyTitan" and myTitan ~= "None") then table.insert(valid, {Name = n, Data = s}) end
	end
	table.sort(valid, function(a, b) return (a.Data.Order or 99) < (b.Data.Order or 99) end)

	for _, sk in ipairs(valid) do
		local c = sk.Data.Type == "Titan" and Color3.fromRGB(160, 40, 40) or (sk.Data.Type == "Style" and Color3.fromRGB(180, 80, 20) or Color3.fromRGB(60, 60, 80))
		local currentCooldown = battleData.Player.Cooldowns and battleData.Player.Cooldowns[sk.Name] or 0
		local disabled = (battleData.Player.TitanEnergy or 0) < (sk.Data.EnergyCost or 0) or currentCooldown > 0
		...
		if sk.Name == "Retreat" then cb = nil end
		...
		if disabled then
			...
			if sk.Name == "Retreat" then
				local isConfirmingFlee = false
				btn.MouseButton1Click:Connect(function()
					if not disabled then
						SFXManager.Play("Click")
						if not isConfirmingFlee then
							isConfirmingFlee = true; btn.Text = "Confirm Retreat?"; btn.BackgroundColor3 = Color3.fromRGB(200, 40, 40)
						...
						else cachedTooltipMgr.Hide(); Network.CombatAction:FireServer("Attack", {SkillName = sk.Name}) end
					end
				end)
			end
		end
...
function StoryTab.UpdateCombat(status, data)
	...
	elseif status == "TurnStrike" then
		...
		if data.DidHit then
			task.spawn(function()
				...
				for i = 1, 6 do combatUI.MainFrame.Position = UDim2.new(0, math.random(-p, p), 0, math.random(-p, p)); task.wait(0.04) end
	...
	elseif status == "Victory" or status == "Defeat" or status == "Fled" then
		...
		for fKey, f in pairs(activeFighters) do f.Frame:Destroy() end
		activeFighters = {}
		...
		elseif status == "Fled" then AddLog("<font color='#AAAAAA'>You safely escaped.</font>", true) end
		task.delay(1.5, function() buttonContainer.Visible = true end)
	end

	if data and data.Battle then
		if data.Battle.Player.Titan and data.Battle.Player.Titan ~= "None" then
			resourceLabel.Text = "TITAN HEAT: " .. math.floor(data.Battle.Player.TitanEnergy or 0) .. " / 100"
		else
			resourceLabel.Text = ""
		end
		...
		if data.Battle.Player.HP <= 0 and activeFighters["Player"] then activeFighters["Player"].Frame:FindFirstChild("InfoArea").NameLabel.Text = data.Battle.Player.Name .. " (KO)" end
